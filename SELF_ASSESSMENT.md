...
| Runnability | 8/10 | Live deploy with a ticking `/pulse` heartbeat + a real `/live` tournament, real Bitget data, four Playbooks, 248 tests — a gated real-money order is one command away (needs a trade key) |
...
248 passing offline tests; the public repo (CI green) and the deploy are both up. **The paper →
