...
.PHONY: setup test cov lint arena firewall firewall-value redteam killswitch allocator funding walkforward overfit-trap bench evidence check-docs verify-evidence integrate playbook-validate serve live live-order tokenized-sweep session-study custom-agent showcase demo verify
...
session-study:   ## Track 3 depth: off-hours vs market-hours risk in tokenized US stocks
	uv run python scripts/tokenized_session_study.py
