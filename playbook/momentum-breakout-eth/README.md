# Momentum Breakout (Donchian, ETH)

A classic breakout strategy (策略) on ETH perpetual futures: it enters when price
breaks beyond its recent trading channel and rides the move, exiting on a tighter
opposite channel that acts as a trailing stop. (Same engine as the BTC breakout
Playbook, deployed to ETH.)
