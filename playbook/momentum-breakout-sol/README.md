# Momentum Breakout (Donchian, SOL)

A classic breakout strategy (策略) on SOL perpetual futures: it enters when price
breaks beyond its recent trading channel and rides the move, exiting on a tighter
opposite channel that acts as a trailing stop. (Same engine as the BTC and ETH
breakout Playbooks, deployed to SOL.)
...
breakouts are mostly false. SOL's higher volatility tends to produce larger, cleaner
breakouts than BTC or ETH — and sharper false ones.
