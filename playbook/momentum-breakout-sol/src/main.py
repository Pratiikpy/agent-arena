"""Entry point for the Momentum Breakout Playbook.

Historical replay: fetch bars -> backtest.run() -> write a REAL equity/NAV curve
(publish anti-fabrication requirement) + a small backtest_report.json with corrected
strategy-basis metrics -> emit a signal. Strategy-agnostic harness; the trading logic
lives in src/strategy.py + backtest.yaml.
"""
...
def run() -> None:
    ...
    try:
        ...
        (OUT / "backtest_report.json").write_text(
            json.dumps(
                {"net_pnl": raw["net_pnl"], "total_return_pct": raw["total_return_pct"], "starting_balance": initial_capital},
                default=str,
            ),
            encoding="utf-8",
        )
