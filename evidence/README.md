...
| `tokenized_session_risk.json` | **Track 3 depth:** off-hours vs regular-session risk per tokenized stock on real 1h candles — **~76% of trading is off-hours**, re-open gaps **1–2% p95** — the dislocation the session firewall gate caps. An overfit-aware test (pooled correlation + permutation baseline, n≈174 transitions) finds the dislocation is **risk/noise, not a tradeable signal** (not statistically significant) — which *validates* cap-not-trade. (DST-aware; US holidays not excluded, which understates off-hours.) |
