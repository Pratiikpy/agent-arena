...
| `tokenized_session_risk.json` | **Track 3 depth:** off-hours vs regular-session risk per tokenized stock on real 1h candles — **~76% of 1h candles are off-hours** (~81% of the 24/7 week), re-open gaps **1.1–2.2% p95** — the dislocation the session firewall gate caps. An overfit-aware test (pooled correlation + permutation baseline, n≈174 transitions) finds the dislocation is **risk/noise, not a tradeable signal** on this sample (not statistically significant) — which *supports* cap-not-trade as the prudent default (a per-run result on a small sample, not a permanent claim). (DST-aware; off-hours share is approximate — hourly-bar granularity, US holidays not excluded.) |
| `last_run/` | Working copy of the most recent tournament — what the API serves at `GET /leaderboard` (and the deployed UI reads) |
| `redteam.json` | 22 adversarial attacks + 3 controls; **0 unsafe orders passed**, all signed |
